...
function TableList() {
  ...
  useEffect(() => {
    try {
      const fetchTableList = async() =>{
        const res = await fetch (`/api/table/allTables/${userId}`,{
          method: "GET",
          headers: {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setTableList((prev)=>{
            const updateList = prev.data.filter((table)=> table._id !== tableDelete)
            return {...prev, data: updateList}
  ...
  const handleReserved = async (tableId) => {
    try {
        const res = await fetch(`/api/table/confirmTable/${tableId}`, {
            method: "PATCH",
            headers: {
                "Content-Type": "application/json",
            },
        ...
        if (res.ok) {
            setTableList((prev) => {
                const updateList = prev.data.map((table)=>{
                    if(table._id === tableId){
                        return {...table, isReserved: !table.isReserved}
                    }
                    return table
                }
                );
                      ...
                      {tableList?.data?.map((order) => (
                        ...
                        <tr key={order._id}>
                          ...
                          <td className="whitespace-nowrap px-2 md:px-4 py-4">
                            ...
                            <button
                                onClick={() => handleReserved(order._id)}
                                className={`${
                                  order.isReserved
                                ...
                                >
                                {order.isReserved ? "Reserved" : "Not Reserved"}
                                </button>
